  ...
  const puxarHistorico = async () => {
    try {
      const response = await fetch(
        `http://127.0.0.1:5000/puxar_historico/${paginaEmpAtiv}/${paginaEmpConc}/${paginaResAtiv}/${paginaMulPend}/${paginaMulConc}`,
        {
          method: "GET",
          headers: {
      ...
      setHistorico((prev) => ({
        emprestimos_ativos: [
          ...(prev?.emprestimos_ativos || []),
          ...result.emprestimos_ativos,
        ],
        emprestimos_concluidos: [
          ...(prev?.emprestimos_concluidos || []),
          ...result.emprestimos_concluidos,
        ],
        reservas_ativas: [
          ...(prev?.reservas_ativas || []),
          ...result.reservas_ativas,
        ],
        multas_pendentes: [
          ...(prev?.multas_pendentes || []),
          ...result.multas_pendentes,
        ],
        multas_concluidas: [
          ...(prev?.multas_concluidas || []),
          ...result.multas_concluidas,
        ],
      }));
    } catch (error) {

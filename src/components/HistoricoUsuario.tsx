...
interface HistoricoUsuarioProps {
    userID: number;
}

const HistoricoUsuario = ({ userID }: HistoricoUsuarioProps) => {
    ...
    const [historico, setHistorico] = useState<Historico>({
        ...
        multas_pendentes: [],
        multas_concluidas: [],
        emprestimos_pendentes: [], // Add initial state for emprestimos_pendentes
    });
    ...
    const puxarHistorico = useCallback(async () => {
        ...
        try {
            ...
            switch (activeTab) {
                case "emprestimosAtivos":
                    endpoint = `historico/${userID}/emprestimos_ativos/${paginaEmpAtiv}`;
                    break;
                case "emprestimosConcluidos":
                    endpoint = `historico/${userID}/emprestimos_concluidos/${paginaEmpConc}`;
                    break;
                case "reservasAtivas":
                    endpoint = `historico/${userID}/reservas_ativas/${paginaResAtiv}`;
                    break;
                case "multasPendentes":
                    endpoint = `historico/${userID}/multas_pendentes/${paginaMulPend}`;
                    break;
                case "multasConcluidas":
                    endpoint = `historico/${userID}/multas_concluidas/${paginaMulConc}`;
                    break;
                case "emprestimosPendentes":
                    endpoint = `historico/${userID}/emprestimos_pendentes/${paginaEmpPend}`;
                    break;
                default:
                    return;
            }
    ...
    }, [activeTab, paginaEmpAtiv, paginaEmpConc, paginaResAtiv, paginaMulPend, paginaMulConc, paginaEmpPend, token, userID]);
    ...
    const [filtros, setFiltros] = useState({
        ...
        multasPendentes: "",
        multasConcluidas: "",
        emprestimosPendentes: "", // Add filter state for emprestimos_pendentes
    });
                    ...
                    <div>
                        ...
                        <div>
                            {historico.emprestimos_pendentes.length > 0 ? (
                                ...
                                <p className="no-data-message montserrat-alternates">Não há empréstimos pendentes.</p>
                            )}
                        </div>
                    </div>
                );

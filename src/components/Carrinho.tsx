...
const Carrinho: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchEmprestimos = async () => {
      try {
        ...
        Swal.fire({
          icon: "error",
          title: "Erro",
          text: "Falha ao buscar empréstimos.",
        });
        navigate("/");
      }
    };
    ...
    const fetchData = async () => {
      if (!isAllowed) return; // Verificação de permissão
      setLoading(true);
      await fetchReservas();
  ...
  return (
    <div>
      ...
      <section>
        ...
        {reservas.length === 0 ? (
          ...
          reservas.map((book) => (
            <div key={book.id_reserva}>
              ...
              <h3>{book.titulo}</h3>
              <p>{book.autor}</p>
              <button onClick={() => removerLivro(book.id_reserva!, "reserva")}>
                Remover
              </button>
            </div>
          ))
        ...
        {reservas.length > 0 && (
          <button onClick={reservarLivros} disabled={reserving}>
            Reservar Todos
          </button>
        )}
      </section>
      ...
      <section>
        ...
        {emprestimos.length > 0 && (
          <button onClick={emprestarLivros} disabled={borrowing}>
            Emprestar Todos
          </button>
        )}
      </section>
